...
try:
    ...
    df_rate = pd.merge(df_race_list, df_rate, on='umaban_id', how='left')
    df_rate = df_rate.sort_values(['日付','race_id'])
    df_rate = df_rate.loc[:, ["race_id","馬","馬番","着順","日付","芝・ダート","コース内外","場id","タイム差","距離","umaban_id","馬番成績"]]
    df_concat = df_rate[ df_rate["馬番成績"].notnull() ]
    df_processing = df_rate[ df_rate["馬番成績"].isnull() ]
except:
    # データの読み込み
    yearStart = 2011
    yearEnd = 2024
    ...
    df_combined["umaban_id"] = df_combined["race_id"].astype(str) +'_'+ df_combined["場id"].astype(str) +'_'+ df_combined["芝・ダート"].astype(str) +'_'+ df_combined["コース内外"].astype(str) +'_'+ df_combined["距離"].astype(str) +'_'+ df_combined["馬番"].astype(str)
    df_processing = df_combined.loc[:, ["race_id","馬","馬番","着順","日付","芝・ダート","コース内外","場id","タイム差","距離","umaban_id"]]
    df_processing = df_processing.sort_values(['日付','race_id'])
    df_processing["馬番成績"]=0.5
    df_rate = df_processing

for i, row in enumerate(df_processing.itertuples()):
    idx = row[0]
    df_processing.loc[idx, "馬番成績"] = 0.5
    ...
    ub = df_processing.loc[idx,"馬番"] #馬番
    n = df_race_id[ df_race_id["race_id"]==race_id ]
    n = n.loc[n.index.values.tolist(),"タイム差"].values.tolist()[0]

    umaban_id = str(race_id) +'_'+ str(c) +'_'+ str(sd) +'_'+ str(us) +'_'+ str(dis) +'_'+ str(ub)
    df_ul = df_rate[ (df_rate["馬番"]==ub) & (df_rate["場id"]==c) & (df_rate["芝・ダート"]==sd) & (df_rate["コース内外"]==us) & (df_rate["距離"]==dis) & (df_rate["race_id"]<race_id) & (df_rate["馬番成績"].notnull()) ]
    df_ul = df_ul.sort_values(['日付'])
    if df_ul.empty==False:
        df_ul = df_ul.tail(1)
        df_processing.loc[idx,"馬番成績"] = df_ul.loc[df_ul.index.values.tolist(),"馬番成績"].values.tolist()[0]
    else:
        df_processing.loc[idx,"馬番成績"] = 0.5

    p = df_processing.loc[idx,"馬番成績"]
    if df_processing.loc[idx,"タイム差"]!=0:
        point = round( (0.2 * (1-(df_processing.loc[idx,"タイム差"]/n))) + (0.8 * p) ,2)
    else:
        point = round( 0.2 + (0.8 * p) ,2)

    df_processing.loc[idx,"馬番成績"] = point
    print(d,umaban_id,point,p,n)
...
df_processing = df_processing.drop(["race_id","馬","馬番","着順","日付","距離","芝・ダート","コース内外","場id","タイム差"],axis=1)

print("データ変換：完了")

# エンコーディングとスケーリング後のデータを確認
print("ファイル出力：開始")
df_processing.to_csv('encoded/umaban_win_rate.csv', index=False)
df_processing.to_csv('encoded/umaban_win_rate_shift.csv', index=False,encoding="SHIFT_JIS")
print("ファイル出力：終了")
