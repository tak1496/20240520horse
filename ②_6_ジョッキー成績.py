...
try:
    ...
    df_rate = pd.merge(df_race_list, df_rate, on='jockey_id', how='left')
    df_rate = df_rate.sort_values(['日付','race_id'])
    df_rate = df_rate.loc[:, ["race_id","騎手","着順","日付","芝・ダート","コース内外","場id","タイム差","距離","jockey_id","騎手成績"]]
    df_concat = df_rate[ df_rate["騎手成績"].notnull() ]
    df_processing = df_rate[ df_rate["騎手成績"].isnull() ]
except:
    # データの読み込み
    yearStart = 2011
    yearEnd = 2024
    ...
    df_combined["jockey_id"] = df_combined["race_id"].astype(str) +'_'+ df_combined["場id"].astype(str) +'_'+ df_combined["芝・ダート"].astype(str) +'_'+ df_combined["コース内外"].astype(str) +'_'+ df_combined["距離"].astype(str) +'_'+ df_combined["騎手"].astype(str)
    df_processing = df_combined.loc[:, ["race_id","騎手","着順","日付","芝・ダート","コース内外","場id","タイム差","距離","jockey_id"]]
    
    df_processing = df_processing.sort_values(['日付','race_id'])
    df_processing["騎手成績"]=0.5
    df_rate = df_processing

for i, row in enumerate(df_processing.itertuples()):
    idx = row[0]
    df_processing.loc[idx, "騎手成績"] = 0.5
    ...
    jk = df_processing.loc[idx,"騎手"] #騎手
    n = df_race_id[ df_race_id["race_id"]==race_id ]
    n = n.loc[n.index.values.tolist(),"タイム差"].values.tolist()[0]

    jockey_id = str(race_id) +'_'+ str(c) +'_'+ str(sd) +'_'+ str(us) +'_'+ str(dis) +'_'+ str(jk)
    df_jl = df_rate[ (df_rate["騎手"]==jk) & (df_rate["場id"]==c) & (df_rate["芝・ダート"]==sd) & (df_rate["コース内外"]==us) & (df_rate["距離"]==dis) & (df_rate["race_id"]<race_id) & (df_rate["騎手成績"].notnull()) ]
    df_jl = df_jl.sort_values(['日付'])
    if df_jl.empty==False:
        df_jl = df_jl.tail(1)
        df_processing.loc[idx,"騎手成績"] = df_jl.loc[df_jl.index.values.tolist(),"騎手成績"].values.tolist()[0]
    else:
        df_processing.loc[idx,"騎手成績"] = 0.5

    p = df_processing.loc[idx,"騎手成績"]
    if df_processing.loc[idx,"タイム差"]!=0:
        point = round( (0.2 * (1-(df_processing.loc[idx,"タイム差"]/n))) + (0.8 * p) ,2)
    else:
        point = round( 0.2 + (0.8 * p) ,2)

    df_processing.loc[idx,"騎手成績"] = point
    print(d,jockey_id,point,p,n)
...
df_processing = df_processing.drop(["race_id","騎手","着順","日付","芝・ダート","コース内外","場id","タイム差","距離"],axis=1)

print("データ変換：完了")

# エンコーディングとスケーリング後のデータを確認
print("ファイル出力：開始")
df_processing.to_csv('encoded/jockey_win_rate.csv', index=False)
df_processing.to_csv('encoded/jockey_win_rate_shift.csv', index=False,encoding="SHIFT_JIS")
print("ファイル出力：終了")
